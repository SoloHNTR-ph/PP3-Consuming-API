...
async function fetchMangaDetails(id) {
  try {
    ...
    document.getElementById("manga-cover").src = manga.images.jpg.large_image_url;
...
async function fetchSearchResults(query) {
  try {
    const response = await fetch(`https://api.jikan.moe/v4/manga?q=${encodeURIComponent(query)}`);
    const data = await response.json();
...
function displayManga(mangaList, mangaContainer) {
  for (let i = 0; i < mangaList.length; i++) {
    ...
    setTimeout(async () => {
      ...
      if (mangaData) {
        ...
        link.addEventListener("click", () => localStorage.setItem("selectedMangaId", mangaData.mal_id));
      }
    }, i * 1000);
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  displayManga(mangaList, mangaContainer);
});
